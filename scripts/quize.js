...
async function fetchQuestions(subject) {
    ...
    try {
        ...
        const questions = data[subject].map(q => {
            const answers = q.options.map((option, index) => new Answer(option, index === q.answer));
...
let time ;
let score = 0;
let markedQuestions = JSON.parse(sessionStorage.getItem('markedQuestions')) || [];
let answers = [];
let choisesSelected = JSON.parse(sessionStorage.getItem('choisesSelected')) || [];
let submit = false;
let flagElement
...
function loadStoredSelections() {
    choisesSelected.forEach(selected => {
        const questionElement = document.querySelector(`[data-question="${selected.question}"]`);
...
function loadStoredQuestions() {
    ...
    if (storedQuestions) {
        ...
        document.getElementById("totalQuestions").textContent = storedQuestions.length;
        displayQuestions();
        loadStoredSelections();
...
function selectAnswer(choice, question) {
    ...
    const existingIndex = choisesSelected.findIndex(item => item.question === question);
...
function calculateScore() {  
    const correctAnswers = questions.map(q => q.answers.find(answer => answer.isCorrect).isCorrect);
    score = answers.reduce((acc, answer, index) => acc + (answer === correctAnswers[index]), 0);
    showResult(score);
}
...
function displayMarkedQuestions() {
    ...
    markedElement.innerHTML = `<div class="rounded-2 mark brownColor p-3 d-flex justify-content-between"> <span>Question ${questionId + 1}</span> <span class="text-danger remove"><i class="fa-solid fa-trash"></i></span> </div>`;
    markedElement.querySelector('.remove').onclick = function (e) {
    e.stopPropagation();
    markedElement.remove();
    flagElement.classList.remove('text-danger')
...
function toggleMarkQuestion() {
    ...
    if (markedIndex !== -1) {
        ...
        document.querySelector(`.marked-question[data-id="${questionId}"]`).remove();
        flagElement.classList.remove('text-danger')
    } else {
        markedQuestions.push(questionId);
        displayMarkedQuestions();
        flagElement.classList.add('text-danger')
    }
